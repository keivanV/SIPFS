...
app.post('/upload', authenticateToken, upload.single('file'), async (req, res) => {
    ...
    try {
        ...
        // console.log("decrypted Ct " , decrypt(ct , key));
        //=============================
...
//     const transactionCounts = [1, 5, 10 ,20];
//     const results = [];
//     let gateway;
  
//     try {
//       gateway = await connectToUserGateway('uu1', 'owner');
//       const network  = await gateway.getNetwork('sipfs');
//       const contract = network.getContract('basic');
  
//       const publicKeyOwner = `-----BEGIN CERTIFICATE-----\n…\n-----END CERTIFICATE-----\n`;
//       const now = new Date().toISOString();
  
//       for (const M of transactionCounts) {
//         let totalTimeMs = 0;
  
//         // Sequentially submit M transactions
//         for (let i = 0; i < M; i++) {
//           const assetId = Math.random().toString(36).substring(2, 8);
...
//           // Clean up as you go (optional)
//           await contract.submitTransaction('DeleteAsset', assetId)
//                         .catch(e => console.error(`Cleanup failed for ${assetId}:`, e.message));
//         }
  
//         const totalTime    = totalTimeMs / 1000;        // seconds
//         const throughput   = M / totalTime;             // tx/sec
//         const avgTransTime = totalTime / M;             // sec/tx
  
//         results.push({
//           transactions:       M,
//           totalTime:          parseFloat(totalTime.toFixed(3)),
//           throughput:         parseFloat(throughput.toFixed(3)),
//           avgTransactionTime: parseFloat(avgTransTime.toFixed(3))
//         });
//       }
  
//       // Save to JSON
//       const outputFile = path.resolve(__dirname, 'performanceResults.json');
...
const { runAllMeasurements } = require('./benchmarks');

app.get('/api/measure', async (req, res) => {
    try {
        const measurements = await runAllMeasurements();
        const formattedResults = measurements.map(m => ({
            message: `Function ${m.function} executed in ${m.time} ms`
        }));
        res.status(200).json({
            success: true,
        ...
        res.status(500).json({
            success: false,
            error: 'Failed to measure chaincode functions',
...
const { measureConcurrentPerformance } = require('./concurrentBenchmark');

app.get('/api/measure-concurrent', async (req, res) => {
    try {
        const results = await measureConcurrentPerformance();
        const formattedResults = [];

        for (const func in results) {
            results[func].forEach(result => {
                formattedResults.push({
                    message: `Function ${func} with ${result.concurrency} concurrent requests executed in ${result.averageTime.toFixed(2)} ms per request (total: ${result.totalTime} ms)`
                });
            });
        }
        ...
        console.error('Concurrent measurement API error:', error);
        res.status(500).json({
            success: false,
            error: 'Failed to measure concurrent chaincode functions',
            details: error.message
        });
    }
});
