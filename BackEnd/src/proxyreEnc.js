...
mongoose.connect('mongodb://192.168.1.5:27017/sipfs', {
...
async function connectToUserGateway(username, userType) {
    ...
    await gateway.connect(ccp, {
        ...
        connectionOptions: {
            requestTimeout: 600000, // 30 seconds (default is 5000 ms)
            pingTimeout: 600000,    // 30 seconds (default is 3000 ms)
            eventHandlerOptions: {
                commitTimeout: 600000, // 30 seconds (default is 5000 ms)
...
function decryptChunk(privateKey, encryptedChunk) {
    const buffer = Buffer.from(encryptedChunk, 'base64');
    return crypto.privateDecrypt(privateKey, buffer);
}


// Function to reconstruct and decrypt the original file
async function reconstructFile(fileName) {
    try {
        const walletWorkerDir = path.resolve(__dirname, 'walletWorker');
        const fileSubDir = path.join(walletWorkerDir, fileName);

        const files = await fs.readdir(fileSubDir);
        const jsonFiles = files.filter(file => file.endsWith('.json'));

        let chunkMetadata = [];

        // Read and parse each JSON file for metadata
        for (const file of jsonFiles) {
        const filePath = path.join(fileSubDir, file);
        try {
            const fileContent = await fs.readFile(filePath, 'utf8');
            const jsonData = JSON.parse(fileContent);
            if (Array.isArray(jsonData)) {
            chunkMetadata = chunkMetadata.concat(jsonData);
            } else {
        ...
        chunkMetadata.sort((a, b) => a.index - b.index);

        const chunks = [];
        for (const { index, cid, enc, privateKey } of chunkMetadata) {
        try {
            console.log(`Downloading and decrypting chunk at index ${index} with CID ${cid}`);

            // Simulate IPFS download (replace with actual download code if needed)
            const encryptedChunk = enc;

            // Decrypt the chunk using the private key
            const decryptedChunk = decryptChunk(privateKey, encryptedChunk);
            chunks.push(decryptedChunk);
        } catch (err) {
            console.error(`Error processing chunk at index ${index}:`, err);
        }
        }

        // Concatenate all decrypted chunks to reconstruct the original file
...
async function reconstructAndReEncryptFile(fileName, newUserPk, newUserSk) {
    try {
        const walletWorkerDir = path.resolve(__dirname, 'walletWorker');
        const fileSubDir = path.join(walletWorkerDir, fileName);
        ...
        try {
            ...
            const ReEncryptedChunk = encryptWithPublicKey(newUserPk , decryptedChunk);

            reChunks.push(ReEncryptedChunk);
        ...
        console.log("Re Encrypted Chunks are : " , reChunks);
...
const CHUNK_SIZE = 200; // 2048-bit RSA key allows a max of 245 bytes of data
...
const hashContent = (buffer) => {
    ...
    hash.update(buffer); // Update the hash with the file buffer
    return hash.digest('hex'); // Return the hash as a hexadecimal string
};

async function connectToUserGatewayWithRetry(username, role, retries = 30, delay = 5000) {
    for (let i = 0; i < retries; i++) {
        try {
            const gateway = await connectToUserGateway(username, role);
            return gateway;
        } catch (error) {

            if (i === retries - 1) throw error; // Throw error if all retries fail
            console.log(`Retry ${error} ${i + 1} for Fabric connection...`);
...
app.post('/upload', authenticateToken, upload.single('file'), async (req, res) => {
    ...
    const gateway = await connectToUserGatewayWithRetry(req.user.username, "owner");
    ...
    try {
        ...
        //==== 1. Create Random Symmetric KEY
        const key = generateRandomAccessKey();  // 256 bits = 32 bytes

        //==== 2. Load Worker Public Keys (for encryption)
        for (let i = 1; i <= 5; i++) {
            const workerKeyPath = path.resolve(__dirname, `./workers/w${i}-keys.json`);
            const workerData = await fs.readFile(workerKeyPath, 'utf8');
            const workerKeys = JSON.parse(workerData);
            const publicKeyPem = workerKeys.publicKey;
            const publicKey = crypto.createPublicKey(publicKeyPem);

            const privateKeyPem = workerKeys.privateKey;
            workerPublicKeys.push(publicKey);
            workerPrivateKeys.push(privateKeyPem);
            workerpk.push(publicKeyPem);
        }

        //==== 3. Split the file into chunks
        const chunks = chunkFile(req.file.buffer, CHUNK_SIZE);
        console.log("[!][upload] chunks are " , chunks);

        //==== 4. Encrypt each chunk with the worker's public key (RSA encryption)
        const encryptedChunks = await Promise.all(
            chunks.map(async (chunk, i) => {
                const workerIndex = i % workerPublicKeys.length;
                try {
                    return encryptWithPublicKey(workerPublicKeys[workerIndex], chunk);
                } catch (error) {
                    console.error(`Error encrypting chunk ${i}:`, error);
                    throw new Error(`Encryption failed for chunk ${i}: ${error.message}`);
                }
            })
        );

        //==== 5. Upload each encrypted chunk to IPFS and get their CID (hash)
        const ipfsHashes = [];
        for (let i = 0; i < encryptedChunks.length; i++) {
            const workerIndex = i % workerPublicKeys.length;
            const workerName = `w${workerIndex + 1}`;

            try {
                const { cid } = await uploadFileToIPFS(encryptedChunks[i], req);
                ipfsHashes.push({
                    index: i,
                    privateKey: workerPrivateKeys[workerIndex],
                    publicKey: workerpk[workerIndex],
                    fileName: fileName,
                    enc: encryptedChunks[i],
                    cid: cid,
                    workerName: workerName
                });
                console.log(`Chunk ${i} uploaded successfully. CID: ${cid}`);
            } catch (error) {
                console.error(`Error uploading chunk ${i} to IPFS:`, error);
                throw new Error(`IPFS upload failed for chunk ${i}: ${error.message}`);
            }
        }
        //==== 6. Save metadata to walletWorker directory
        const walletWorkerDir = path.resolve(__dirname, './walletWorker');
        const fileSubDir = path.join(walletWorkerDir, fileName);
        ...
        for (let i = 0; i < ipfsHashes.length; i++) {
            const workerIndex = i % workerPublicKeys.length;
            if (!workerDataMap[workerIndex]) {
                workerDataMap[workerIndex] = [];
            }
            ...
            const dataToWrite = {
                ...
                publicKey: workerpk[workerIndex],
                fileName: fileName,
                cid: ipfsHashes[i].cid,
                enc: encryptedChunks[i],
                workerName: ipfsHashes[i].workerName
            };

            const duplicate = workerDataMap[workerIndex].find(entry => entry.index === dataToWrite.index);
            if (!duplicate) {
                workerDataMap[workerIndex].push(dataToWrite);
            } else {
                console.log(`Duplicate entry for index ${dataToWrite.index} for worker-${workerIndex + 1} found; skipping.`);
            }
        }

        for (const workerIndex in workerDataMap) {
            const workerFilePath = path.join(fileSubDir, `worker-${Number(workerIndex) + 1}.json`);
            await fs.writeFile(workerFilePath, JSON.stringify(workerDataMap[workerIndex], null, 2));
            console.log(`Wrote data for worker-${Number(workerIndex) + 1} to ${workerFilePath}`);
        }

        //==== 7. Process policy attributes (shamir and access control)
        ...
        const fileHash = hashContent(req.file.buffer);
        ...
        // Split the access key using Shamir's Secret Sharing
        ...
        //==== 8. Connect to Hyperledger Fabric and submit transaction
        ...
        try {
            let PrevCid = "";
            if (version == "demo") {
                const hashAcessKey = bcrypt.hashSync(key);

                // Example transaction submission (uncomment if needed)
                const result = await submitTransactionWithRetry(
                    contract,
                    'CreateAsset',
                    [assetId, JSON.stringify(metadata), JSON.stringify(policyAttributes), publicKeyOwner, currentDate, currentDate, req.user.username, fileName, fileHash, PrevCid, hashAcessKey, JSON.stringify(fragmentsMap), JSON.stringify(hashedAttributes)],
                    5, // retries
                    1000 // delay
                );
            }
        } catch (error) {
            console.error("Failed to submit transaction to Hyperledger Fabric:", error);
            throw new Error("[x] Failed to submit transaction to Hyperledger Fabric");
        } finally {
            gateway.disconnect();
        }

        //==== 9. Save asset to MongoDB
        try {
            ...
            console.error("Failed to save asset in MongoDB:", error);
            throw new Error("[x] Failed to save asset in MongoDB");
        }

        //==== 10. Reconstruct and save the file
        try {
            const RequserKeyFilePath = path.resolve(__dirname, `./keys/${req.user.username}-keys.json`);
            const reqdata = await fs.readFile(RequserKeyFilePath, 'utf8');
            const requserKeys = JSON.parse(reqdata);

            const privateKey = crypto.createPrivateKey(requserKeys.privateKey);
            const publicKey = crypto.createPublicKey(requserKeys.publicKey);

            const fileBuffer = await reconstructAndReEncryptFile(fileName, publicKey, privateKey);
            const outputFilePath = path.resolve(__dirname, fileName);
            await fs.writeFile(outputFilePath, fileBuffer);
            console.log(`File successfully reconstructed and saved to: ${outputFilePath}`);
        } catch (error) {
...
//             let workerIndex = i % workerPublicKeys.length;  // Cyclically use each worker's public key
            
//             // Wrap the encryption in a promise
//             const encryptionPromise = new Promise((resolve, reject) => {
//                 try {
//                     const encryptedChunk = encryptWithPublicKey(workerPublicKeys[workerIndex], chunks[i]);
//                     resolve(encryptedChunk);
//                 } catch (error) {
//                     reject(`Error encrypting chunk ${i}: ${error.message}`);
//                 }
//             });
        
//             // Push the promise into the array
//             encryptedChunks.push(encryptionPromise);
//         }
        
//         // Wait for all promises to resolve
//         const resolvedChunks = await Promise.all(encryptedChunks);
//         encryptedChunks = resolvedChunks.map(chunk => chunk);
...
//         for (let i = 0; i < encryptedChunks.length; i++) {
//             // Identify which worker node is responsible for this chunk (based on index)
//                 workerIndex = i % workerPublicKeys.length;  // Cycle through workers
//             const workerName = `w${workerIndex + 1}`; // Worker names: w1, w2, ..., w5
        
//             // Upload encrypted chunk to IPFS
            
//             const { cid } = await uploadFileToIPFS(encryptedChunks[i], req);
        
//             // Store both CID and corresponding worker name
//             ipfsHashes.push({
//                 index: i, 
//                 privateKey: workerPrivateKeys[workerIndex],
//                 publicKey: workerpk[workerIndex],
//                 fileName: fileName,
//                 enc:encryptedChunks[i],
//                 cid: cid,           // IPFS CID for the chunk
//                 workerName: workerName // Corresponding worker name (w1, w2, w3, etc.)
//             });
//         }
        
//         const walletWorkerDir = path.resolve(__dirname, './walletWorker');
//         // Create a subdirectory under walletWorker named after the uploaded file
//         const fileSubDir = path.join(walletWorkerDir, fileName);
//         await fs.mkdir(fileSubDir, { recursive: true });
...
//         // Loop over each ipfsHashes entry.
//         for (let i = 0; i < ipfsHashes.length; i++) {
//             const workerIndex = i % workerPublicKeys.length;
            
//             // Initialize the array for this worker if it doesn't exist.
//             if (!workerDataMap[workerIndex]) {
//                 workerDataMap[workerIndex] = [];
//             }
...
//             // Optionally, check for a duplicate index in the array (though if ipfsHashes is unique, this may not occur).
//             const duplicate = workerDataMap[workerIndex].find(entry => entry.index === dataToWrite.index);
//             if (!duplicate) {
//                 workerDataMap[workerIndex].push(dataToWrite);
//             } else {
...
//         // Now, for each worker, write the data to its JSON file, overwriting any existing content.
//         for (const workerIndex in workerDataMap) {
//             const workerFilePath = path.join(fileSubDir, `worker-${Number(workerIndex) + 1}.json`);
//             await fs.writeFile(workerFilePath, JSON.stringify(workerDataMap[workerIndex], null, 2));
//             console.log(`Wrote data for worker-${Number(workerIndex) + 1} to ${workerFilePath}`);
...
//         //==== 6. Process policy attributes (shamir and access control)
//         let policyAttributesArray = [];
...
//             let PrevCid = "";
//             if (version == "demo") {
//                 // const assetData = await contract.evaluateTransaction('GetAssetsByOwnerAndName', req.user.username, req.file.originalname, "DEMO");
...
//             //     const newgateway = await connectToUserGateway(req.user.username, "owner");
//             //     const newnetwork = await newgateway.getNetwork(channelName);
//             //     const newcontract = newnetwork.getContract(chaincodeName);
...
//         console.log("asset id " , assetId , "publicKey " , publicKeyOwner);
//         try {
//             const newAsset = new WareHouse({
//                 fileId: assetId,
//                 publicKey: publicKeyOwner,
...
//         try {
//             // const fileBuffer = await reconstructFile(fileName);
...
//             const privateKeyPem = requserKeys.privateKey;

//             const privateKey = crypto.createPrivateKey(privateKeyPem);


//             const publicKeyPem = requserKeys.privateKey;

//             const publicKey = crypto.createPrivateKey(publicKeyPem);
...
// Utility function to chunk a file
...
app.post('/promote', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        console.error("Error in /promoteAccess:", error);
...
app.post('/grantAccess', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        // console.log(decrypt(ct  , result.key));
...
app.put('/assets/:id' , authenticateToken ,   async (req, res) => {
    ...
    try {
        ...
        let policyAttributesArray = existingAsset.policySet;

        // if (typeof existingAsset.policySet === 'string') {
        //         try {
        //             policyAttributesArray = JSON.parse(existingAsset.policySet); // Parse the string into an object/array
        //         } catch (error) {
        //             return res.status(400).json({
        //                 success: false,
        //                 message: "Invalid policyAttributes format. Could not parse the string."
        ...
        // console.log("policyAttributesArray "  , policyAttributesArray)
        ...
        //------- TEST SHAMIR ------
        // console.log("policyAttributes : " , typeof JSON.stringify(policyAttributes) );
        // console.log('Hashed Attributes:', typeof JSON.stringify(hashedAttributes));
        ...
        console.log("[!][UPADTE] new FragementsMAP " , fragmentsMap);
        ...
        if (existingAsset.type == "DEMO")
        {
            ...
            await contract.submitTransaction('UpdateAsset', assetId, JSON.stringify(metaData), JSON.stringify(policySet), publicKeyOwner, releasedAt, currentDate , existingAsset.owner , existingAsset.name ,existingAsset.cid , existingAsset.PrevCid , existingAsset.hashAccessKey , JSON.stringify(fragmentsMap) , JSON.stringify(hashedAttributes));
