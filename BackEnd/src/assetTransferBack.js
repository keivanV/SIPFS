...
class AssetTransferABAC extends Contract {
    ...
    async CreateAsset(ctx, id, metaData, policySet, publicKeyOwner, releaseAt, updatedAt, owner, name, EncCid, PrevCid) {
    ...
    async CreateFullAsset(ctx, id, metaData, policySet, publicKeyOwner, releaseAt, updatedAt, owner, name, EncCid, PrevCid) {
        const exists = await this.AssetExists(ctx, id);
        if (exists) {
            throw new Error(`The asset ${id} already exists`);
        }
        ...
        const asset = {
            ...
            ReleasedAt: releaseAt,
            UpdatedAt: updatedAt,
            Requesters_pending_level1: [],
            Requesters_level1: [],
            Requesters_Revoked_level1: [],
            Requesters_level2: [],
            signed: [],
            owner: owner,
            name: name,
    ...
    async CreateAssetPublic(ctx, id, metaData, policySet, publicKeyOwner, releaseAt, updatedAt , owner , name , version) {
        const exists = await this.AssetExists(ctx, id);
        if (exists) {
            throw new Error(`The asset ${id} already exists`);
        }
    ...
    async UpdateAsset(ctx, id, metaData, policySet, publicKeyOwner, releaseAt, updatedAt , owner , name , cid , PrevCid ) {
        const exists = await this.AssetExists(ctx, id);
        if (!exists) {
            throw new Error(`The asset ${id} does not exist`);
        }
    ...
    async UpdateAssetFull(ctx, id, metaData, policySet, publicKeyOwner, releaseAt, updatedAt , owner , name , cid , PrevCid ) {
        const exists = await this.AssetExists(ctx, id);
        if (!exists) {
            throw new Error(`The asset ${id} does not exist`);
        }
        ...
        const updatedAsset = {
            ...
            Requesters_pending_level1: [],
            Requesters_level1:  [],
            Requesters_Revoked_level1 : [], // first level promotion
            Requesters_level2 : [], // FULL PROMOtion
            signed : [],
            owner : owner,
            name : name,
            cid : cid,
    ...
    async CheckAccess(ctx, username, assetID) {
        ...
        return { access: userHasAccess, owner: asset.owner };
    }
